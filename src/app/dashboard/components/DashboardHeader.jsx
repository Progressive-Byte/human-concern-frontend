...
const DashboardHeader = ({ title, subtitle, actions }) => {
  ...
  return (
    <>
      ...
      {menuOpen && (
        <nav className="lg:hidden bg-white border-b border-dashed border-[#E5E7EB] px-4 py-3 space-y-1">
          {navItems.map((item) => {
          ...
          <div className="pt-2 border-t border-dashed border-[#E5E7EB] space-y-1 mt-1">
            <Link
              href="/donate"
              onClick={() => setMenuOpen(false)}
              className="flex items-center gap-2 w-full rounded-2xl bg-[#1A1A1A] px-3 py-3 text-sm font-medium text-white hover:bg-[#333333] transition-colors"
            >
              {MakeDonerIcon}
              Make a Donation
            </Link>
