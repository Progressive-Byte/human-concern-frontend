import ObjectiveRowActions from "./ObjectiveRowActions";
  ...
  return (
    <section className="hc-animate-fade-up hc-hover-lift rounded-2xl border border-dashed border-[#E5E7EB] bg-white">
      <div className="px-5 py-4">
        <h2 className="text-[18px] font-semibold text-[#111827]">All Objectives</h2>
      </div>
      ...
      {loading ? (
        ...
        <div className="px-5 py-10 text-center text-sm text-[#6B7280]">No objectives found</div>
      ) : (
          ...
          <table className="w-full min-w-[980px] border-collapse">
            <thead>
              <tr className="text-left text-[12px] font-medium text-[#6B7280]">
                ...
                <th className="py-3 pr-4">Ramadan Only</th>
            ...
            <tbody className="text-[13px] text-[#111827]">
              {rows.map((item) => (
                ...
                >
                  <td className="px-5 py-4 align-top">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {item?.slug ? <div className="mt-1 text-[12px] text-[#6B7280]">{item.slug}</div> : null}
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-4 pr-4 align-top">{item?.ramadanOnly ? yesPill("Yes") : <span className="text-[#6B7280]">—</span>}</td>

                  <td className="py-4 pr-4 align-top">
                    <ObjectiveStatusPill status={item?.status} />
                  </td>

                  <td className="py-4 pr-5 align-top text-right">
                    <ObjectiveRowActions item={item} onEdit={onEdit} onArchive={onArchive} onRestore={onRestore} />
                  </td>
                </tr>
              ))}
            </tbody>
