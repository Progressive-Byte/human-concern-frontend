...
function getConfirmCopy(action) {
  if (action === "archive") return { title: "Archive objective?", description: "Archived objectives are removed from active use.", confirmText: "Archive" };
  if (action === "restore") return { title: "Restore objective?", description: "This will restore the objective from archived state.", confirmText: "Restore" };
  return { title: "Confirm action", description: "", confirmText: "Confirm" };
}

const ObjectiveRowActions = ({ item, onEdit, onArchive, onRestore }) => {
...
export default ObjectiveRowActions;