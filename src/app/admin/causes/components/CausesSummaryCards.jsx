...
function CoinIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5">
      ...
      <path d="M5 7v10c0 1.66 3.13 3 7 3s7-1.34 7-3V7" stroke="currentColor" strokeWidth="2" />
    </svg>
  );
}
  ...
  return (
    <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
      {loading ? (
        ...
        <>
          <KpiCard label="Total Causes" value={Number(s.total || 0)} icon={<HeartIcon />} iconPosition="left" />
          <KpiCard label="Enabled" value={Number(s.enabled || 0)} icon={<CheckIcon />} iconPosition="left" />
          <KpiCard label="Zakat Eligible" value={Number(s.zakatEligible || 0)} icon={<CoinIcon />} iconPosition="left" />
        </>
      )}
    </section>
  );
