...
const PromisedVsCollectedChart = ({ items = [] }) => {
  ...
  return (
    <div className="hc-animate-fade-up hc-hover-lift rounded-2xl border border-dashed border-[#E5E7EB] bg-white p-5">
      <div className="mb-4">
        <h2 className="text-[16px] font-semibold text-[#111827]">Goal vs Collected</h2>
      </div>
            ...
            <BarChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 10 }} barCategoryGap={22}>
              ...
              <Tooltip
                formatter={(value, name) => {
                  ...
                  const label = key.includes("goal") ? "Goal" : key.includes("collect") ? "Collected" : String(name || "");
              ...
              />
              <Bar dataKey="goal" name="Goal" fill="var(--admin-accent-200)" radius={[6, 6, 0, 0]} barSize={22} animationDuration={500} />
              <Bar dataKey="collected" name="Collected" fill="var(--admin-accent-600)" radius={[6, 6, 0, 0]} barSize={22} animationDuration={500} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
