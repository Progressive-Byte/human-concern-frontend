...
export default function AdminForgotPasswordPage() {
  ...
  async function handleSubmit(e) {
    ...
    setServerError("");
    try {
      await adminRequestPasswordReset({ email: email.trim() });
      setSuccess(true);
    } catch (err) {
    ...
    return (
      <div className="text-center py-4">
        ...
        <p className="text-sm text-white/60 mb-6">
          ...
          <span className="text-white/80">{email}</span>, we've sent a reset link.
        </p>
  ...
  return (
    <div>
      ...
      <p className="text-center text-[13px] text-white/50 mt-5">
        <Link href="/admin/login" className="text-white/80 font-medium hover:text-white transition-colors">
          Back to sign in
        </Link>
      </p>
    </div>
