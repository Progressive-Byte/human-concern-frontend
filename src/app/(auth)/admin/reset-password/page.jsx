...
function ResetPasswordForm() {
  ...
  async function handleSubmit(e) {
    ...
    setServerError("");
    try {
      await adminResetPassword({ token, newPassword: values.newPassword });
      setSuccess(true);
    } catch (err) {
  ...
  if (!token) {
    return (
      <div className="text-center py-4">
        ...
        <Link href="/admin/forgot-password" className="text-[13px] text-white/80 font-medium hover:text-white transition-colors">
          Request a new link
        </Link>
      </div>
    );
  ...
  if (success) {
    return (
      <div className="text-center py-4">
        ...
        <p className="text-sm text-white/60 mb-6">Your admin password has been reset successfully.</p>
        <Link href="/admin/login" className="text-[13px] text-white/80 font-medium hover:text-white transition-colors">
          Sign in
        </Link>
      </div>
    );
  ...
  return (
    <div>
      ...
      <p className="text-[13px] text-white/50 mb-6">Choose a strong password for your admin account.</p>
